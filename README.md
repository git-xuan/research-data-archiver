...
| 页面 | 做什么 |
|---|---|
| **① 初始化归档目录** | 选一个空目录作为归档根，自动创建 **5 个一级分类 / 30 个二级分类**（可选三级）共 115+ 个文件夹，并写入《归档说明.md》与归档索引。一个根目录只需初始化一次 |
...
| **④ 数据台账** | 每次归档后**自动生成《科研数据元信息采集》里对应的一行**，可导出**列序与官方文件完全一致**的 xlsx，直接复制粘贴回官方表 |
