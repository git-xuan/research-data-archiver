...
| 页面 | 做什么 |
|---|---|
| **① 初始化归档目录** | 选一个空目录作为归档根，在分类树里**勾选要创建的分类**——一级 / 二级 / 三级都能只挑一部分，不勾的不会建出来；勾了子目录会自动带上其父目录。**三级分类默认不勾选**（勾『同时列出三级分类子目录』只是把它们显示出来，不会顺手建一堆目录）。支持「全选 / 全不选」、实时显示"将创建 一级 x 个 · 二级 y 个 · 三级 z 个 · 合计 n 个"，需要补建时再勾上点『补齐勾选的分类目录』，已建目录与已有数据不受影响。同时写入《归档说明.md》（只列实际创建的目录）与归档索引 |
| **② 文件归档** | 扫描文件夹或拖入文件 → 在列表里勾选 → 选分类 → 复制归档。分类下拉列出的是**归档目录里实际创建过的分类**（不是完整分类树），所以不会把文件归档进没建过的目录；**如果该二级分类下建了三级目录，会多出一个「三级分类」下拉，必须一并选择**，文件落到 一级/二级/三级；该二级下没建三级时三级下拉收起，直接落到二级。**不删除原始文件**；已归档的文件显示 `[已归档]` 且不可再勾选，**不会重复复制**；目标目录已有同名同大小文件时按已归档直接跳过 |
| **③ 归档自查** | 检查范围可精确到**任意一 / 二 / 三级子文件夹**；支持「全部文件」或**按分类随机抽样**，**抽样比例、每类保底数、每类封顶数都由用户自定**；归档里建了三级目录时，会出现『抽样分组』下拉，可选**按二级分类**或**按三级分类**分组各自独立抽取（没落到三级目录的文件仍归入其二级分组）。勾选问题文件后一键导出 CSV 清单 |
| **④ 数据台账** | 每次归档后**自动生成《科研数据元信息采集》里对应的一行**，可导出**列序与官方文件完全一致**的 xlsx，直接复制粘贴回官方表；归档到三级目录时，官方表里的『三级分类（选填）』列也会一并填上。表格 10 列保证可读宽度，放不下时底部有**横向滚动条**，也可拖动表头调宽 |

> **三级分类不是必须的**：初始化时一个三级都不勾，就得到一个纯两级归档区，归档页不会出现三级下拉，
> 自查页也不会出现「按三级分类」选项。只有真的建了三级目录，这两处才会出现并要求按三级走。
...
| 初始化时展开三级、逐个挑选 |
|---|
| ![三级可选](screenshots/8-初始化-三级可选.png) |
...
```
归档根目录/
├── 文献与情报数据/            ← 一级分类（勾选了几个就建几个）
│   ├── 学术文献/              ← 二级分类
│   │   ├── 期刊论文/          ← 三级分类（可选，默认不建；勾了才会创建）
│   │   └── 会议论文/
│   └── ...
├── 实验与过程数据/
├── 表征与测试数据/
